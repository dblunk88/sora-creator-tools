...
(function () {
  ...
  // == Mobile performance: disable feed/grid autoplay ==
  ...
  function disableMobilePreviewAutoplay(videoEl) {
    try {
      if (!videoEl || videoEl.nodeType !== 1) return;
      if (videoEl.hasAttribute(SCT_MOBILE_AUTOPLAY_DISABLED_ATTR)) return;
      videoEl.setAttribute(SCT_MOBILE_AUTOPLAY_DISABLED_ATTR, '1');
  ...
  function processMobileAutoplayNodes() {
    sctMobileAutoplayScheduled = false;
    if (!sctMobileAutoplayActive) {
      sctMobileAutoplayPending.clear();
      return;
    }
    try {
      for (const n of sctMobileAutoplayPending) {
        try {
          if (!n || n.nodeType !== 1) continue;
          if (n.tagName === 'VIDEO') disableMobilePreviewAutoplay(n);
          const vids = n.querySelectorAll ? n.querySelectorAll('video') : null;
          if (vids && vids.length) {
  ...
  function scheduleMobileAutoplayProcess() {
    if (sctMobileAutoplayScheduled) return;
    sctMobileAutoplayScheduled = true;
    ...
    sctMobileAutoplayObs = new MutationObserver((records) => {
      ...
      try {
        for (const r of records) {
          ...
          for (const n of added) {
            if (!n || n.nodeType !== 1) continue;
            sctMobileAutoplayPending.add(n);
          }
        }
      } catch {}
