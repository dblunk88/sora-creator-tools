...
(function initSoraUVDraftsPageModule(globalScope) {
  ...
  function createSoraUVDraftsPageModule(deps = {}) {
    ...
    function getDraftPostUrl(draft) {
      if (uvDraftsLogic && typeof uvDraftsLogic.getDraftPostUrl === 'function') {
        return uvDraftsLogic.getDraftPostUrl(draft, SITE_ORIGIN);
      }
      const postId = String(draft?.post_id || '').trim();
      if (!postId) return '';
      return `${SITE_ORIGIN}/p/${encodeURIComponent(postId)}`;
    }
    ...
    function getDraftTrimUrl(draft) {
      if (uvDraftsLogic && typeof uvDraftsLogic.getDraftTrimUrl === 'function') {
        return uvDraftsLogic.getDraftTrimUrl(draft, SITE_ORIGIN);
      }
      if (!draft?.id) return '';
      if (draft.storyboard_id) return `${SITE_ORIGIN}/storyboard/${encodeURIComponent(draft.storyboard_id)}`;
      return `${SITE_ORIGIN}/d/${encodeURIComponent(draft.id)}`;
    }
  ...
  async function fetchAllUVDrafts(onProgress) {
    ...
    do {
	      const url = new URL('/backend/project_y/profile/drafts', SITE_ORIGIN);
      ...
      try {
        ...
        if (cursor && (!data.items || data.items.length === 0)) {
	          const retryUrl = new URL('/backend/project_y/profile/drafts', SITE_ORIGIN);
  ...
  async function sendReadRequest(draftId) {
    try {
      ...
      const res = await fetch(`${SITE_ORIGIN}/backend/project_y/profile/drafts/${draftId}/read`, {
        method: 'POST',
        credentials: 'include',
        headers
      });
  ...
  const UV_DRAFTS_PENDING_ENDPOINT = `${SITE_ORIGIN}/backend/nf/pending/v2`;
  ...
  function startComposerFlow(mode, statusEl) {
    ...
    if (mode === 'compose') {
      ...
      window.location.href = `${SITE_ORIGIN}/drafts`;
      return;
    }
    ...
    if (mode === 'remix') {
      if (source?.id) {
        ...
        window.location.href = `${SITE_ORIGIN}/d/${source.id}?remix=`;
        return;
      }
    ...
    if (mode === 'extend') {
      if (source?.storyboard_id) {
        ...
        window.location.href = `${SITE_ORIGIN}/storyboard/${source.storyboard_id}`;
      } else if (source?.id) {
        if (promptForRemix) {
          sessionStorage.setItem('SORA_UV_REDO_PROMPT', promptForRemix);
        }
  ...
  function createUVDraftCard(draft) {
    ...
    const draftUrl = `${SITE_ORIGIN}/d/${encodeURIComponent(draft.id)}`;
    ...
    actionsRow.appendChild(createActionBtn(icons.trash, 'Delete', async () => {
      ...
      try {
        ...
        const res = await fetch(`${SITE_ORIGIN}/backend/project_y/profile/drafts/${draft.id}`, {
          method: 'DELETE',
          credentials: 'include',
    ...
    postBtn.addEventListener('click', async (e) => {
      ...
      try {
        ...
        const res = await fetch(`${SITE_ORIGIN}/backend/project_y/posts`, {
          method: 'POST',
          credentials: 'include',
          headers: postHeaders,
          body: JSON.stringify({
